...
#define LIBM_DD(name) \
  {(cfunction_ptr)f_ ## name,  #name, 1, 1, 0},
#define LIBM_DD_NO(name)
...
#include "libm.h"
#ifdef HAVE_FREXP
  {(cfunction_ptr)f_frexp,"frexp", 1, 1, 0},
#endif
#ifdef HAVE_MODF
  {(cfunction_ptr)f_modf,"modf", 1, 1, 0},
#endif
#ifdef HAVE_LGAMMA_R
  {(cfunction_ptr)f_lgamma_r,"lgamma_r", 1, 1, 0},
#endif
  ...
  {(cfunction_ptr)f_dump, "tojson", 1, 1, 0},
  {(cfunction_ptr)f_json_parse, "fromjson", 1, 1, 0},
  {(cfunction_ptr)f_tonumber, "tonumber", 1, 1, 0},
  {(cfunction_ptr)f_tostring, "tostring", 1, 1, 0},
  {(cfunction_ptr)f_keys, "keys", 1, 1, 0},
  {(cfunction_ptr)f_keys_unsorted, "keys_unsorted", 1, 1, 0},
  {(cfunction_ptr)f_startswith, "startswith", 2, 1, 0},
  {(cfunction_ptr)f_endswith, "endswith", 2, 1, 0},
  {(cfunction_ptr)f_ltrimstr, "ltrimstr", 2, 1, 0},
  {(cfunction_ptr)f_rtrimstr, "rtrimstr", 2, 1, 0},
  {(cfunction_ptr)f_string_split, "split", 2, 1, 0},
  {(cfunction_ptr)f_string_explode, "explode", 1, 1, 0},
  {(cfunction_ptr)f_string_implode, "implode", 1, 1, 0},
  {(cfunction_ptr)f_string_indexes, "_strindices", 2, 1, 0},
  {(cfunction_ptr)f_setpath, "setpath", 3, 1, 0}, // FIXME typechecking
  {(cfunction_ptr)f_getpath, "getpath", 2, 1, 0},
  {(cfunction_ptr)f_delpaths, "delpaths", 2, 1, 0},
  {(cfunction_ptr)f_has, "has", 2, 1, 0},
  ...
  {(cfunction_ptr)f_contains, "contains", 2, 1, 0},
  {(cfunction_ptr)f_length, "length", 1, 1, 0},
  {(cfunction_ptr)f_utf8bytelength, "utf8bytelength", 1, 1, 0},
  {(cfunction_ptr)f_type, "type", 1, 1, 0},
  {(cfunction_ptr)f_isinfinite, "isinfinite", 1, 1, 0},
  {(cfunction_ptr)f_isnan, "isnan", 1, 1, 0},
  {(cfunction_ptr)f_isnormal, "isnormal", 1, 1, 0},
  {(cfunction_ptr)f_infinite, "infinite", 1, 1, 0},
  {(cfunction_ptr)f_nan, "nan", 1, 1, 0},
  {(cfunction_ptr)f_sort, "sort", 1, 1, 0},
  {(cfunction_ptr)f_sort_by_impl, "_sort_by_impl", 2, 0, 0},      // pure, but not useful to mark it so
  {(cfunction_ptr)f_group_by_impl, "_group_by_impl", 2, 0, 0},    // pure, but not useful to mark it so
  {(cfunction_ptr)f_min, "min", 1, 0, 0},
  {(cfunction_ptr)f_max, "max", 1, 0, 0},
  {(cfunction_ptr)f_min_by_impl, "_min_by_impl", 2, 0, 0},
  {(cfunction_ptr)f_max_by_impl, "_max_by_impl", 2, 0, 0},
  {(cfunction_ptr)f_error, "error", 2, 0, 0},  // actually pure, but we can't handle errors at bind time
  {(cfunction_ptr)f_format, "format", 2, 1, 0},
  {(cfunction_ptr)f_env, "env", 1, 1, 1},                         // pure-ish
  {(cfunction_ptr)f_halt, "halt", 1, 0, 1},                       // DEFINITELY NOT PURE
  {(cfunction_ptr)f_halt_error, "halt_error", 2, 0, 0},           // DEFINITELY NOT PURE
  {(cfunction_ptr)f_get_search_list, "get_search_list", 1, 1, 1}, // pure-ish
  {(cfunction_ptr)f_get_prog_origin, "get_prog_origin", 1, 1, 1}, // pure-ish
  {(cfunction_ptr)f_get_jq_origin, "get_jq_origin", 1, 0, 1},     // pure-ish
  {(cfunction_ptr)f_match, "_match_impl", 4, 1, 0},
  {(cfunction_ptr)f_modulemeta, "modulemeta", 1, 0, 0},           // pure, but must be run at run-time
  {(cfunction_ptr)f_input, "_input", 1, 0, 1},                    // DEFINITELY NOT PURE
  {(cfunction_ptr)f_debug, "debug", 1, 0},                        // DEFINITELY NOT PURE
  {(cfunction_ptr)f_stderr, "stderr", 1, 0},                      // DEFINITELY NOT PURE
  {(cfunction_ptr)f_strptime, "strptime", 2, 1, 0},
  {(cfunction_ptr)f_strftime, "strftime", 2, 1, 0},
  {(cfunction_ptr)f_strflocaltime, "strflocaltime", 2, 0, 0},
  {(cfunction_ptr)f_mktime, "mktime", 1, 1, 0},
  {(cfunction_ptr)f_gmtime, "gmtime", 1, 1, 0},
  {(cfunction_ptr)f_localtime, "localtime", 1, 0, 0},
  {(cfunction_ptr)f_now, "now", 1, 0, 0},                         // DEFINITELY NOT PURE
  {(cfunction_ptr)f_current_filename, "input_filename", 1, 0, 1}, // pure, but we won't have this info at the right time
  {(cfunction_ptr)f_current_line, "input_line_number", 1, 0, 1},  // pure, but we won't have this info at the right time
};
#undef LIBM_DDDD_NO
#undef LIBM_DDD_NO
#undef LIBM_DD_NO
