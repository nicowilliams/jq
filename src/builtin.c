...
static jv f_inputs_step(jq_state *jq, void *state) {
  jq_input_cb cb;
  void *data;
  jq_get_input_cb(jq, &cb, &data);
  if (cb == NULL)
...
static jv f_inputs(jq_state *jq, void **state, jv input) {
  jv_free(input);
  *state = NULL;
...
static void f_explodes_reset(jq_state *jq, void *data) {
  struct f_explodes_state *state = data;
  if (state) {
    jv_free(state->str);
    free(state);
  }
...
static jv f_explodes_step(jq_state *jq, void *data) {
  struct f_explodes_state *state = data;
  assert(state != NULL);
...
static jv f_explodes(jq_state *jq, void **data, jv input) {
...
static jv f_readfile_step(jq_state *jq, void *state) {
  FILE *f = state;
  jv out;
...
static jv f_readfile(jq_state *jq, void **state, jv input) {
  FILE *f = *state;
  jv out;
