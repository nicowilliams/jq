...
#include "libm.h"
  ...
  {(cfunction_ptr)f_write, "_write", 3},
  {(cfunction_ptr)f_fopen, "_fopen", 3},
  {(cfunction_ptr)f_popen, "_popen", 3},
...
static const char* const jq_builtins[] = {
  ...
  "def stdin: while(eof(0)|not)|_read(0; null)|if length == 0 then empty elif length == 2 then . else .[0] end;",
  "def stdin(flags): while(eof(0)|not)|_read(0; flags)|if length == 0 then empty elif length == 2 then . else .[0] end;",
  "def read(handle; flags): while(eof(handle)|not)|_read(handle; flags)|if length == 0 then empty elif length == 2 then . else .[0] end;",
  "def write(handle): _write(handle; {});",
  "def write(handle; flags): _write(handle; flags);",
  "def write(handle; flags; generator): generator|_write(handle; flags);",
  // XXX Make these close (replace with null handle) on backtrack (how?)
  "def fopen: _fopen(.; {});",
  "def fopen(opts): _fopen(.; opts);",
  "def fopen(handle; opts): _fopen(handle; opts);",
  "def popen: _fopen(.; {});",
  "def popen(opts): _fopen(.; opts);",
  "def popen(handle; opts): _fopen(handle; opts);",
  "def fread: read(_fopen(.; {}));",
  "def fread(opts): read(_fopen(.; opts));",
  "def fread(fname; opts): read(_fopen(fname; opts));",
  "def exec: read(_popen(.; {}));",
  "def exec(opts): read(_popen(.; opts));",
  "def exec(cmd; opts): read(_popen(cmd; opts));",
#include "libm.h"
