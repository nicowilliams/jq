...
// If parser == NULL -> RAW
jq_util_input_state jq_util_input_state_init(jq_err_cb err_cb, void *err_cb_data) {
  if (err_cb == NULL) {
    err_cb = fprinter;
    err_cb_data = stderr;
  }
...
void jq_util_input_state_set_parser(jq_util_input_state state, jv_parser *parser, int slurp) {
...
void jq_util_input_state_free(jq_util_input_state *state) {
...
void jq_util_input_state_add_input(jq_util_input_state state, jv fname) {
  state->files = jv_array_append(state->files, fname);
}

int jq_util_input_state_open_errors(jq_util_input_state state) {
...
int jq_util_input_state_read_more(jq_util_input_state state) {
...
jv jq_util_input_state_next_input_cb(jq_state *jq, void *data) {
  return jq_util_input_state_next_input((jq_util_input_state)data);
}

// Blocks to read one more input from stdin and/or given files
// When slurping, it returns just one value
jv jq_util_input_state_next_input(jq_util_input_state state) {
  int is_last = 0;
  jv value = jv_invalid(); // need more input
  do {
    if (state->parser == NULL) {
      // Raw input
      is_last = jq_util_input_state_read_more(state);
      if (state->buf[0] == '\0')
        continue;
