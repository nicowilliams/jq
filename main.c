...
int main(int argc, char* argv[]) {
  ...
  jq_util_input_state input_state = jq_util_input_state_init(NULL, NULL); // XXX add err_cb
  ...
  for (int i=1; i<argc; i++, short_opts = 0) {
    if (further_args_are_files) {
      jq_util_input_state_add_input(input_state, jv_string(argv[i]));
...
#define isatty _isatty
  ...
  if (!(options & RAW_INPUT))
    jq_util_input_state_set_parser(input_state, jv_parser_new(parser_flags), (options & SLURP) ? 1 : 0);

  // Let jq program read from inputs
  jq_set_input_cb(jq, jq_util_input_state_next_input_cb, input_state);

  // Let jq program call `debug` builtin and have that go somewhere
  jq_set_debug_cb(jq, debug_cb, &dumpopts);

  if (first_file == 0)
    jq_util_input_state_add_input(input_state, jv_string("-"));
  if (options & PROVIDE_NULL) {
    ret = process(jq, jv_null(), jq_flags, dumpopts);
  } else {
    jv value;
    while (jq_util_input_state_open_errors(input_state) == 0 &&
           (jv_is_valid((value = jq_util_input_state_next_input(input_state))) || jv_invalid_has_msg(jv_copy(value)))) {
  ...
  if (ret == 0 && jq_util_input_state_open_errors(input_state) != 0)
    ret = 2;
...
out:
  jq_util_input_state_free(&input_state);
  jq_teardown(&jq);
